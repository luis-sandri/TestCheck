  ...
  return <div className="app-shell"><aside className="sidebar">
    ...
    <section className="nonconformity-list">
      ...
      {nonconformities.map((nonconformity) => <article className="panel nonconformity-card" key={nonconformity.id}>
        <div className="nc-header"><div><span className="case-code">{nonconformity.code} · {nonconformity.test_case_code}</span><h2>{nonconformity.test_case_title}</h2><p>{nonconformity.description}</p></div><div className="nc-badges"><span className={`status ${nonconformity.severity === 'HIGH' ? 'danger' : nonconformity.severity === 'MEDIUM' ? 'warning' : 'success'}`}>{severityLabel[nonconformity.severity]}</span><span className="status warning">{statusLabel[nonconformity.status]}</span></div></div>
        <p className="nc-meta">Responsável: {nonconformity.assignee_email} · Prazo: {nonconformity.due_date || 'não definido'}</p>
        {nonconformity.status !== 'RESOLVED' && (nonconformity.can_submit_evidence || nonconformity.can_review) && <button className="text-button" disabled={sending === nonconformity.id} type="button" onClick={() => void retryNotification(nonconformity)}>Reenviar e-mail</button>}
